...
def setStdLevel(level):
	"""设置标准日志输出等级
		...
		less：		精简等级，只会输出错误信息。使用此等级不会向终端输出帖子信息，但其它方式的帖子记录没有影响"""
	global __STDLEVEL
...
def stdLog(message, logtype):
	"""标准日志输出
		参数为message和level，message为将要输出的信息，level为日志等级，当日志等级高于当前设置的等级时，将向对应的设备输出信息
		使用此函数不会记录调用栈"""
...
def __logToFile(message, logtype, time):
	try:
		...
		__errToConsole(201, getStack(), getLogTime(), pause = True)
...
def postLog(theadData, method = None, APIKey = ''):
	...
	if APIKey != '' and 'cloud' in method:
		#发送日志并判断是否成功
		if __postToCloud(theadData, APIKey) == False:
			#失败时输出错误信息
...
def __postToCloud(theadData, APIKey):
    ...
    r = requests.post("http://tieba.yangwang.hk/tieba/post.php", data = postdata)
    if r.content == 'poi':
        return True
    else:
        return False
