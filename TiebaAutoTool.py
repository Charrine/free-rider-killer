...
def configFileGenerator():
	filename = raw_input(u'请输入配置文件的文件名（按回车使用默认文件）: ')
	if filename == '':
	...
	if os.path.exists(filename):
		inputs = raw_input(u'文件已存在，本操作将覆盖此文件，是否继续？（y继续操作）: ')
	...
	isLogined = False
	while not isLogined:
		config['user']['username'] = raw_input(u'请输入用户名: ').decode(config['stdincoding'])
		config['user']['password'] = getpass.getpass(u'请输入密码（无回显）: ')
		...
		if not config['debug']:
			...
			if not isLogined:
				print u'-----登陆失败-----'
				inputs = raw_input(u'按q可退出,回车继续尝试: ')
				if inputs in ['q', 'Q']:
	...
	config['forum']['kw'] = raw_input(u'请输入贴吧名（不带‘吧’，如c语言吧则输入‘c语言’）: ').decode(config['stdincoding'])

	print u'-----------------\n请检查输入的信息是否正确'
	print u'使用用户名：{0}\n密码：{1}\n管理贴吧：{2}'.format(config['user']['username'], '*' * len(config['user']['password']), config['forum']['kw'])
	inputs = raw_input(u'输入yes将会写入用户配置文件: ')
...
def init():
	...
	if config['workingType'] == 'autoTool':
		...
		print u'管理贴吧： ' + config['forum']['kw'] + u'(' + config['forum']['fid'] + u')'

	return

def getUserConfigration():
	print u'获取配置文件：' + config['configFilename']+ '...\n'
...
def getKeywords():
	...
	try:
		...
		print u'无法得到 keywords，文件可能不存在或者格式可能不对'
		sys.exit(1)
